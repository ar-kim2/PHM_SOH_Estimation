from SOHestimatior_discharge import SOHestimator_discharge
import os
import numpy as np
...
Model_Dir = os.path.join(Model_Dir, 'SOH_iter2000_seq20_unit5_f2_predict1_drop05_0523_input4_{}'.format(testcase))
Estimator = SOHestimator_discharge(data_load=True, feature_size=4, drop_out=True, test_case=testcase) # SOH 추정 모델 클래스 로드
...
def test_data_make(result_list, input_list, future_len, seq_len, pred_cyc):
    ...
    # test_sample = np.load("./data/battery_device/Data_Excel_1224/8_H_Battery/test_sample.npy", allow_pickle=True)
