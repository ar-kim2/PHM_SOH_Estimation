...
""" Battery의 충전 데이터를 통해 SOH 추정하는 모델 학습 """
class SOHestimator_charge:
    ...
    def __init__(self, data_load=True, feature_size=None, drop_out=None, test_case=0):
        ...
        self.batch_size = 2048  # input 배치사이즈 [ Batch size, sequnece legnth, feature size(2) ]  # 256  # 512
        self.seq_length = 20
        ...
        self.ent_min = 0.597
        self.ent_max = 1.064
        self.charge_min = 629.391
        self.charge_max = 4157.996
        self.current_min = 0.0
        self.current_max = 2.21
        self.current_min2 = 0.7835718
        self.current_max2 = 1.0893813
        self.current_min3 = 0.7835718
        self.current_max3 = 1.0893813
        ...
        self.log_path = "./Model"
        self.result_path = \
            os.path.join(self.log_path,"SOH_iter{}_seq{}_unit{}_f{}_predict{}_drop03_0524_charge_input4_3"\
    ...
    def data_loader(self, battery_list, isTraining = False):
        ...
        for battery in battery_list:  # 배터리 하나
            ...
            discharge_data = np.load(BatteryDataDir + '/charge_data.npy', allow_pickle=True)
            DC_list_current = np.load(BatteryDataDir + '/charge_current.npy', allow_pickle=True)
            DC_time = np.load(BatteryDataDir + '/charge_time_all.npy', allow_pickle=True)
            ...
            for cycle in range(len(discharge_data[:degenerated_cycle])):
                try:
                    voltage_idx_end = np.where(np.array(discharge_data[cycle]) > end_vol)[0][-1] + 1
                except:
                    print("Check end Except!!!")
                    voltage_idx_end = -1

                if voltage_idx_end == 0:
                    voltage_idx_end = 1

                discharge_data[cycle] = list(discharge_data[cycle][:voltage_idx_end])
                DC_list_current[cycle] = list(DC_list_current[cycle][:voltage_idx_end])
                DC_time[cycle] = list(DC_time[cycle][:voltage_idx_end])
        ...
        DC_Entropy, DC_charge, DC_prob, _, _, DC_mul, DC_Cur_Entropy = Entropy.EntropyForSOHProb_total_cur_charge(all_DC_data, all_DC_current_data, 'discharge', all_SOH_data, all_DC_time)
    ...
    def load_data(self):
        ...
        # for train
        SOH_train_data, CAP_train_data, DC_train_Entropy, DC_train_charge, DC_train_prob, C_train_entropy, DC_train_current_avg, DC_train_current_var, DC_train_mul, DC_train_current_ent = self.data_loader(self.train_battery_list, isTraining=True)
        # for validation
        ...
        for i, train_data in enumerate(self.InputEntProb):
            ...
            if battery.__contains__('CS2_') and battery.__contains__('/'):
                battery_path = battery.split('/')
                BatteryDataDir = os.path.join(dir_path_CS2_change, battery_path[0])
                BatteryDataDir = os.path.join(BatteryDataDir, battery_path[1])
                train_sample = np.load(BatteryDataDir + '/train_sample.npy', allow_pickle=True)
            #             elif battery.__contains__('CS2_'):
            #                 if self.test_case == 2 or self.test_case == 4:
            #                     BatteryDataDir = os.path.join(dir_path_CS2, battery)
            #                     train_sample = np.load(BatteryDataDir + '/train_sample.npy', allow_pickle=True)
        ...
        #print("Check shape1 : ", len(self.TrainP1_1))
...
if __name__ == "__main__":
    Estimator = SOHestimator_charge(feature_size=4, data_load=True, drop_out=True)
    Estimator.train()
